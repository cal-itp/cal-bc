...
def project_benefit(request: HtmxHttpRequest, **kwargs) -> HttpResponse:
    ...
    with io.BytesIO() as buffer:
        ...
        buffer.seek(0)
        new_calculator = Calculator(io.BytesIO(buffer.read()))

    benefit = new_calculator.compile().evaluate("3) Results!H13")
    return render(request, "projects/benefits/show.html", {"benefit": benefit})


class ProjectBenefitDetailView(LoginRequiredMixin, UpdateView):
    template_name = "projects/benefits/show.html"
    model = Project
