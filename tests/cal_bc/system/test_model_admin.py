...
class TestModelAdmin(StaticLiveServerTestCase):
    ...
    def test_model_admin(self):
        ...
        self.page.locator(".app-models").get_by_role("rowheader", name="Models").get_by_role("link", name="Models", exact=True).click()
        ...
        self.page.locator(".app-models").get_by_role("link", name="Versions", exact=True).click()
        ...
        self.page.get_by_role("button", name="Save", exact=True).click()
        self.page.wait_for_selector("text=The version “Sketch v8.1” was added successfully")

        self.page.get_by_role("link", name="Home").click()
        self.page.locator(".app-models").get_by_role("link", name="Versions", exact=True).click()
        self.page.get_by_role("link", name="Sketch v8.1", exact=True).click()
        self.page.locator(":text('Section: #1') + fieldset").get_by_label("Name").nth(0).fill("Project Information")
        ...
        self.page.locator(":text('Values') ~ table tbody tr").nth(0).locator("td").nth(1).locator("input").fill("District 4 - Bay Area")
        self.page.locator(":text('Values') ~ table tbody tr").nth(0).locator("td").nth(2).locator("input").fill("District 4")
        self.page.get_by_role("button", name="Save", exact=True).click()
        self.page.wait_for_selector("text=The group “Sketch v8.1 § 1A General Information” was changed successfully")
